...
// Demo product data for each category (10 products each)
const productData = {
  'Dream Catcher': [
    {
...
async function addDemoProducts() {
  ...
  try {
    // First, clear existing products (optional - remove this if you want to keep existing products)
    ...
    for (const [category, products] of Object.entries(productData)) {
      ...
      for (let i = 0; i < products.length; i++) {
        ...
        const productToInsert = {
          ...
          price: product.price,
          category: category,
          catalog_number: `${category.replace(/\s+/g, '').toUpperCase()}-${String(i + 1).padStart(3, '0')}`,
    ...
    console.log(`\n🎉 Successfully added ${totalAdded} demo products!`);
    console.log('Products have been organized into the following categories:');
