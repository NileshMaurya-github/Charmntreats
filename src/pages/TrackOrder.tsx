    ...
    try {
      ...
      if (orderData) {
        ...
        const timeline = [
          ...
          {
            status: 'Shipped',
            date: orderData.order_status === 'shipped' || orderData.order_status === 'delivered' ? new Date().toLocaleDateString() : 'Pending',
            time: orderData.order_status === 'shipped' || orderData.order_status === 'delivered' ? 'Shipped' : 'Pending',
            completed: orderData.order_status === 'shipped' || orderData.order_status === 'delivered',
            description: 'Package has been dispatched'
          },
          {
            status: 'Delivered',
            date: orderData.order_status === 'delivered' ? new Date().toLocaleDateString() : 'Expected',
            time: orderData.order_status === 'delivered' ? 'Delivered' : 'Expected',
            completed: orderData.order_status === 'delivered',
            description: orderData.order_status === 'delivered' ? 'Package delivered successfully' : 'Package will be delivered soon'
          }
        ];

        setTrackingResult({
          orderNumber: orderData.order_number,
          status: orderData.order_status.charAt(0).toUpperCase() + orderData.order_status.slice(1),
          estimatedDelivery: new Date(Date.now() + 3 * 24 * 60 * 60 * 1000).toLocaleDateString(),
          currentLocation: orderData.order_status === 'delivered' ? 'Delivered' : 'Processing Center',
          timeline: timeline,
          orderData: orderData
        });
      } else {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900">
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="flex items-center gap-4 mb-8">
            ...
            <div>
              ...
              <p className="text-slate-900">Enter your order number to track your package</p>
            </div>
          </div>
              ...
              <CardHeader>
                ...
                <div className="flex-1">
                  <div className="flex items-center justify-between">
                    <h3 className={`font-medium ${item.completed ? 'text-slate-900' : 'text-slate-800'}`}>
                      {item.status}
                    </h3>
                    <span className={`text-sm ${item.completed ? 'text-slate-900' : 'text-slate-700'}`}>
                      {item.date} {item.time !== 'Expected' && `at ${item.time}`}
                    </span>
                  </div>
                  <p className={`text-sm ${item.completed ? 'text-slate-900' : 'text-slate-700'}`}>
                    {item.description}
                  </p>
                </div>
              </div>
      ...
      {user && userOrders.length > 0 && !trackingResult && (
        <Card className="bg-white border border-slate-200 shadow-sm mb-8">
          <CardHeader>
            <CardTitle className="text-slate-900">Your Recent Orders</CardTitle>
            <p className="text-slate-900">Click on any order to track it quickly</p>
          </CardHeader>
          <CardContent>
              ...
              {userOrders.slice(0, 5).map((order: any) => (
                ...
                >
                  ...
                  <div className="text-right">
                    ...
                    <span className={`text-xs px-2 py-1 rounded-full border ${order.order_status === 'delivered' ? 'bg-green-100 text-green-700 border-green-200' :
                      order.order_status === 'shipped' ? 'bg-blue-100 text-blue-700 border-blue-200' :
                        order.order_status === 'processing' ? 'bg-yellow-100 text-yellow-700 border-yellow-200' :
